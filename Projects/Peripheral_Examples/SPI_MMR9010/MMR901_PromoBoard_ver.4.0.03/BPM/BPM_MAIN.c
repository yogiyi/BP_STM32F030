...
void bpm_display(void)
{
	switch(bpm_ctrl.phase){
		...
		// -----------------------------
			bpm_ctrl.phase = 0x12;
			bpm_manage.countdown5ms = M_T5MS_COUNT_10S;							//表示タイマーを再設定
			lcd_data.error = bpm_manage.errorCode;
			lcd_ctrl.mode  = LCD_MODE_ERR_UPDATE;
			break;
		...
		// -----------------------------
			if(bpm_manage.countdown5ms == 0x00){								// TIMEOUT ?
				...
				bpm_manage.keyRequest = BPM_KEY_REQ_NONE;						//表示内容の切替要求クリア
				bpm_ctrl.phase = 0xFF;
			}
			break;
		case 0xFF:
...
void bpm_calc_blood_vessel_age(void)
{
	...
	bpm_bva_para.ph_temp[0] = 0;												// s[0] = m[0]+…+m[N-1] = 0*N = 0
	...
	bpm_bva_para.ph_max[0] = bpm_bva_para.ph_temp[0] * BPM_MOVE_AVE_NUM;		// s[0] = m[0]*N = m[0]+…+m[N-1]
